...
import { isSoundEnabled, playSound, resumeAudioContext } from "@/lib/sounds";
...
export function StartupScreen({ onLogin }: StartupScreenProps) {
  ...
  const handleLogin = () => {
    ...
    if (userData.code === userCode.trim()) {
      toast.success(
        t("auth.success.loggedIn").replace("{username}", userData.username),
      );
      playSound("gameStart");
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <main className="flex-1 container mx-auto px-4 py-6 sm:py-8 flex flex-col items-center justify-center gap-6">
          ...
          >
            <Card
              className={`${getCardClass()} border-2 transform transition-all duration-300 hover:scale-[1.02] hover:shadow-3xl`}
            >
              {mode === "select" && (
                <>
                  <CardHeader className="text-center space-y-3">
                    ...
                    <CardDescription
                      className={`text-base sm:text-lg ${theme === "light" ? "text-gray-600" : "text-gray-300"}`}
                    >
                      {t("auth.selectOption")}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4 p-6">
                    <Button
                      onClick={() =>
                        handleButtonClick(() => setMode("register"))
                      }
              ...
              {mode === "register" && generatedCode && (
                <>
                  <CardHeader className="space-y-2">
                    ...
                    <CardDescription
                      className={`text-center text-sm sm:text-base ${theme === "light" ? "text-gray-600" : "text-gray-300"}`}
                    >
                  ...
                  <CardContent className="space-y-5 p-6">
                    ...
                    <div className="space-y-2">
                      <Label className="text-base">
                        {t("auth.register.yourCode")}
                      </Label>
                      <div
                        className={`p-4 rounded-lg font-mono text-xl font-bold text-center ${
                          theme === "light"
              ...
              {mode === "login" && (
                <>
                  ...
                  <CardContent className="space-y-5 p-6">
                    <div className="space-y-2">
                      <Label htmlFor="userCode" className="text-base">
                        {t("auth.login.code")}
                      </Label>
                      <Input
                        id="userCode"
