...
export function generateBlockSet(playerLevel = 1): BlockShape[] {
  ...
  for (
    let complexity = levelConfig.difficulty.minShapeComplexity;
    complexity <= levelConfig.difficulty.maxShapeComplexity;
    complexity++
  ) {
    const indices =
      COMPLEXITY_MAP[complexity as keyof typeof COMPLEXITY_MAP] || [];
    availableIndices.push(...indices);
  }

  // Filter to valid indices
  const validIndices = availableIndices.filter(
    (idx) => idx < BLOCK_SHAPES.length,
  );

  // Fallback to basic shapes if no valid indices
  const shapesToUse =
    validIndices.length > 0 ? validIndices : [0, 1, 2, 3, 4, 5];
