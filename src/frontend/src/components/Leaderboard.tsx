...
export function Leaderboard({
  ...
  const getRowClass = (isCurrentUser: boolean) => {
    const baseClass =
      "flex items-center gap-2 sm:gap-3 md:gap-4 p-2 sm:p-3 rounded-lg transition-colors";
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent
        className={`${getDialogClass()} max-w-2xl max-h-[85vh] sm:max-h-[90vh]`}
      >
        <DialogHeader>
                ...
                return (
                  <div key={entry.userCode}>
                    <div className={getRowClass(isCurrentUser)}>
                      <div
                        className={`flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-bold text-sm sm:text-base ${getRankBadgeClass(index + 1)}`}
                      >
                        {index + 1}
                      </div>
