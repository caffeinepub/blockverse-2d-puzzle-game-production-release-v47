...
import { ExternalLink, Key, LogOut, User, X } from "lucide-react";
...
export function UserProfile({ theme, onClose, onLogout }: UserProfileProps) {
  ...
  const handlePrivacyClick = () => {
    playSound("button");
    window.open(
      "https://sites.google.com/view/blockverseapp/privacy-policy",
      "_blank",
      "noopener,noreferrer",
    );
  };
    ...
    >
      ...
      >
        <CardHeader className="text-center">
          <CardTitle
            id="profile-title"
            className={`text-2xl sm:text-3xl font-bold ${getTextClass()}`}
          >
            {t("profile.title")}
          </CardTitle>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-4">
            <div
              className="flex items-start gap-3 p-4 rounded-lg bg-opacity-50"
              style={{
                backgroundColor:
                  theme === "light"
            ...
            >
              <User
                className={`h-5 w-5 mt-0.5 flex-shrink-0 ${getIconClass()}`}
              />
              <div className="flex-1 min-w-0">
                ...
                <p
                  className={`text-lg font-bold ${getTextClass()} break-words`}
                >
                  {user.username}
                </p>
              </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p
                  className={`text-lg font-bold font-mono ${getTextClass()} break-all`}
                >
                  {user.code}
                </p>
              </div>
