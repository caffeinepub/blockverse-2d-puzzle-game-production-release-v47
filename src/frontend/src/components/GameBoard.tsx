...
import type { FallingBlock, MovingObstacle } from "@/pages/Game";
...
export function GameBoard({
  ...
  const isObstacleAt = useCallback(
  ...
  const getCellClass = (row: number, col: number) => {
    ...
    const isPreview =
      ...
      selectedBlock.shape[row - hoverPosition.row][col - hoverPosition.col] ===
        1;
    ...
    const fallingBlock = fallingBlocks.find(
      (fb) => fb.row === row && fb.col === col,
    );

    let baseClass =
      "game-cell aspect-square rounded-sm sm:rounded-md transition-all duration-100";
    ...
    if (hasObstacle) {
      ...
      switch (theme) {
        ...
        case "neon":
          baseClass +=
            " bg-purple-600/80 border-2 border-pink-400 shadow-lg shadow-purple-500/50";
          break;
      }
    }
    ...
    // Regular occupied cell
      ...
      if (activePowerUp === "blockBreak" && isHovered) {
        baseClass +=
          " ring-4 ring-red-500 ring-offset-2 scale-110 animate-pulse";
      }
      ...
      if (selectedBlock) {
        ...
        switch (theme) {
          ...
          case "neon":
            baseClass +=
              " border-pink-300 shadow-xl shadow-[0_0_20px_rgba(236,72,153,0.7)]";
            break;
        }
      }
    } else if (isPreview && !canPlace) {
      ...
      switch (theme) {
        ...
        case "neon":
          baseClass +=
            " bg-black/40 hover:bg-black/60 border border-pink-500/20";
          break;
      }
    }
  ...
  const getBoardClass = () => {
    const glowIntensity = levelConfig.visualEffects.glowIntensity;
    let baseClass =
      "game-board bg-gradient-to-br backdrop-blur-sm shadow-2xl border-4";
    ...
    if (activePowerUp === "blockBreak") {
      ...
      baseClass +=
        " ring-4 ring-yellow-500 ring-offset-4 ring-offset-transparent";
    } else if (activePowerUp === "rowBreak") {
      baseClass +=
        " ring-4 ring-orange-500 ring-offset-4 ring-offset-transparent";
    }
