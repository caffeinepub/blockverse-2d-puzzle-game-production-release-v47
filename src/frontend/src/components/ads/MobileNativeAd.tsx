...
export function MobileNativeAd({ theme, className = "" }: MobileNativeAdProps) {
  ...
  const getContainerClass = () => {
    let baseClass =
      "w-full rounded-lg overflow-hidden p-3 sm:p-4 min-h-[120px] ";
    ...
    switch (theme) {
      ...
      case "neon":
        baseClass +=
          "bg-gradient-to-br from-purple-900/50 to-pink-900/50 border border-pink-500/30";
        break;
    }
  ...
  return (
    <div ref={containerRef} className={`${getContainerClass()} ${className}`} />
  );
}
