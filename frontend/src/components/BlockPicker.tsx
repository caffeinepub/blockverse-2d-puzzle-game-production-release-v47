...
export function BlockPicker({ blocks, selectedIndex, onSelectBlock, disabled, theme }: BlockPickerProps) {
    const getCardClass = () => {
        switch (theme) {
            case 'light':
                        ...
                        >
                            {block.shape.map((row, rowIndex) =>
                                row.map((cell, colIndex) => {
                                    ...
                                    const hasTop = rowIndex > 0 && block.shape[rowIndex - 1][colIndex] === 1;
                                    const hasBottom = rowIndex < rows - 1 && block.shape[rowIndex + 1][colIndex] === 1;
                                    const hasLeft = colIndex > 0 && block.shape[rowIndex][colIndex - 1] === 1;
                                    const hasRight = colIndex < cols - 1 && block.shape[rowIndex][colIndex + 1] === 1;
