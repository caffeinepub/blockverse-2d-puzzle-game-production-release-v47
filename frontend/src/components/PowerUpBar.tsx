...
import { getPowerUps, consumePowerUp, addPowerUp, getPowerUpImage, type PowerUp } from '@/lib/leaderboard';
import { playSound } from '@/lib/sounds';
import { showRewardedAd, isNativeMobile, getRandomPowerUpType } from '@/lib/mobileAds';
...
export function PowerUpBar({ theme, userCode, onUsePowerUp, activePowerUp }: PowerUpBarProps) {
    const { t } = useLanguage();
    ...
    const getButtonClass = (isActive: boolean, count: number) => {
        const baseClass = 'relative flex flex-col items-center justify-center gap-0.5 p-1 xs:p-1.5 sm:p-2 rounded-md sm:rounded-lg transition-all duration-200 touch-manipulation flex-shrink-0';
        
        if (count === 0) {
            return `${baseClass} ${
                theme === 'light' ? 'bg-gray-200 text-gray-400' :
                theme === 'dark' ? 'bg-gray-800 text-gray-600' :
        ...
        if (isActive) {
            return `${baseClass} ${
                theme === 'light' ? 'bg-purple-600 text-white shadow-lg shadow-purple-500/50' :
                theme === 'dark' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/50' :
                'bg-pink-600 text-white shadow-lg shadow-pink-500/50'
            } scale-105`;
        }

        return `${baseClass} ${
            theme === 'light' ? 'bg-white/90 text-purple-700 hover:bg-purple-100' :
            theme === 'dark' ? 'bg-gray-800/90 text-cyan-400 hover:bg-gray-700' :
                            ...
                            <Tooltip key={type}>
                                <TooltipTrigger asChild>
                                    ...
                                    >
                                        ...
                                        <div className={`absolute -top-0.5 -right-0.5 sm:-top-1 sm:-right-1 ${getCountBadgeClass()} rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center text-[8px] xs:text-[9px] sm:text-[10px] font-bold`}>
                                            {count}
                                        </div>
                                    </button>
                                </TooltipTrigger>
                                <TooltipContent>
                                    <p className="font-semibold">{t(`powerups.${shortKey}.name`)}</p>
                                    <p className="text-sm">{t(`powerups.${shortKey}.description`)}</p>
                                    {count === 0 && !isOffline && (
                                        <p className="text-xs mt-1 text-orange-400">{t('powerups.clickToRefill')}</p>
                                    )}
                                    {count === 0 && isOffline && (
                                        <p className="text-xs mt-1 text-orange-400">{t('offline.adsDisabled')}</p>
                                    )}
                                </TooltipContent>
                            </Tooltip>
                        );
            ...
            <Dialog open={showRefillDialog} onOpenChange={setShowRefillDialog}>
                <DialogContent className={getDialogClass()}>
                    <DialogHeader>
                        ...
                        <DialogDescription className={theme === 'dark' ? 'text-gray-400' : theme === 'neon' ? 'text-pink-300' : ''}>
                            {refillType && `${t('ads.refillDescription').replace('{name}', t(`powerups.${refillType}.name`))}`}
                        </DialogDescription>
                    </DialogHeader>
