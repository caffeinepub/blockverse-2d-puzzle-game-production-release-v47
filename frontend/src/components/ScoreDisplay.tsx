...
export function ScoreDisplay({ score, bestScore, comboLines = 0, theme }: ScoreDisplayProps) {
    const { t } = useLanguage();
    ...
    return (
        <div className="relative w-full">
            ...
            <div className="flex flex-row items-center justify-center gap-1.5 sm:gap-3 md:gap-4">
                <Card className={`flex-1 min-w-0 transition-all duration-300 ${getCardClass()} ${getScoreBorderClass()}`}>
                    <div className="p-1.5 sm:p-3 md:p-4 flex items-center justify-between gap-1 sm:gap-2">
                        <div className="flex items-center gap-0.5 sm:gap-2 min-w-0 flex-shrink">
                            <Star className={`flex-shrink-0 w-2.5 h-2.5 sm:w-4 sm:h-4 md:w-5 md:h-5 fill-current ${theme === 'neon' ? 'text-pink-400' : 'text-yellow-500'}`} />
                            <span className={`text-[9px] xs:text-[10px] sm:text-sm font-semibold ${getTextClass()} whitespace-nowrap overflow-hidden text-ellipsis`}>
                                {t('score.current')}
                            </span>
                        </div>
                        <span className={`flex-shrink-0 text-xs xs:text-sm sm:text-lg md:text-xl font-bold ${getScoreColorClass()}`}>{score}</span>
                    </div>
                </Card>

                <Card className={`flex-1 min-w-0 transition-all duration-300 ${getCardClass()} ${getBestBorderClass()}`}>
                    <div className="p-1.5 sm:p-3 md:p-4 flex items-center justify-between gap-1 sm:gap-2">
                        <div className="flex items-center gap-0.5 sm:gap-2 min-w-0 flex-shrink">
                            <Trophy className={`flex-shrink-0 w-2.5 h-2.5 sm:w-4 sm:h-4 md:w-5 md:h-5 fill-current ${theme === 'neon' ? 'text-purple-400' : theme === 'dark' ? 'text-blue-400' : 'text-purple-500'}`} />
                            <span className={`text-[9px] xs:text-[10px] sm:text-sm font-semibold ${getTextClass()} whitespace-nowrap overflow-hidden text-ellipsis`}>
                                {t('score.best')}
                            </span>
                        </div>
                        <span className={`flex-shrink-0 text-xs xs:text-sm sm:text-lg md:text-xl font-bold ${getScoreColorClass()}`}>{bestScore}</span>
                    </div>
                </Card>
            </div>
            ...
            {showCombo && (
                <div className="absolute -bottom-12 sm:-bottom-16 left-1/2 -translate-x-1/2 animate-bounce z-10">
                    <Card className={`transition-all duration-300 ${getComboClass()} shadow-xl sm:shadow-2xl`}>
