...
export function LevelTransition({ newLevel, theme, onComplete }: LevelTransitionProps) {
    const { t } = useLanguage();
    ...
    return (
        <div className={`fixed inset-0 z-50 flex items-center justify-center transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'} ${getOverlayClass()}`}>
                ...
                <Card className="bg-white/10 backdrop-blur-xl border-4 border-white/30 shadow-2xl animate-in zoom-in-95">
                    <div className="p-8 sm:p-12 md:p-16 flex flex-col items-center gap-6">
                        ...
                        <div className="text-center space-y-2">
                            <h2 className={`text-4xl sm:text-5xl md:text-6xl font-bold ${getTextClass()} drop-shadow-lg`}>
                                {t('level.levelUp')}
                            </h2>
                            <div className="flex items-center justify-center gap-2">
                                <Star className="w-6 h-6 sm:w-8 sm:h-8 text-yellow-300 fill-yellow-300" />
                                <p className={`text-2xl sm:text-3xl md:text-4xl font-bold ${getTextClass()}`}>
                        ...
                        {/* Rewards message */}
                        <p className={`text-lg sm:text-xl ${getTextClass()} text-center max-w-md`}>
                            {t('level.newFeatures')}
                        </p>
