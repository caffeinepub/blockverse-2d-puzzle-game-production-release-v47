...
export function shouldShowInterstitialAd(): boolean {
  ...
  const gameCount = parseInt(localStorage.getItem(gameCountKey) || '0', 10);
...
// Track ad events
export function trackMobileAdEvent(eventType: 'shown' | 'clicked' | 'rewarded', adType: string): void {
...
export function getGameCompletionCount(): number {
  const gameCountKey = 'blockverse-completed-games-count';
  return parseInt(localStorage.getItem(gameCountKey) || '0', 10);
}
...
export function preloadAds(): void {
  ...
  if ((window as any).MobileAds) {
    (window as any).MobileAds.preloadInterstitialAd(MOBILE_AD_CONFIG.interstitialAdId);
